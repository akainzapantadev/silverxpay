...
class MY_Controller extends CI_Controller {
    ...
    function _getRecords($tables, $fieldName, $where, $join, $joinType, $sortBy, $sortOrder, $limit, $fieldNameLike, $like, $whereSpecial) {
        $rows = $this->arldevModel->getRecords($tables, $fieldName, $where, $join, $joinType, $sortBy, $sortOrder, $limit, $fieldNameLike, $like, $whereSpecial);
        return $rows;
    }
    function _getRecordsData($data, $tables, $fieldName, $where, $join, $joinType, $sortBy, $sortOrder, $limit, $fieldNameLike, $like, $whereSpecial, $groupBy) {
        $rows = $this->arldevModel->getRecordsData($data, $tables, $fieldName, $where, $join, $joinType, $sortBy, $sortOrder, $limit, $fieldNameLike, $like, $whereSpecial, $groupBy);
        return $rows;
    }
    function _updateRecords($tableName, $fieldName, $where, $data) {
        $rows = $this->arldevModel->updateRecords($tableName, $fieldName, $where, $data);
        return $rows;
    }
    function _insertRecords($tableName, $data) {
        $rows = $this->arldevModel->insertRecords($tableName, $data);
        return $rows;
    }
    function _deleteRecords($tableName, $fieldName, $where) {
        $rows = $this->arldevModel->deleteRecords($tableName, $fieldName, $where);
        return $rows;
    }
